...
export default function TransitionScribble() {
  useEffect(() => {
    ...
    const runScribbleAnimation = (e) => {
      ...
      const drawTl = gsap.timeline({
        onComplete: () => {
          ...
          // Hide the whole overlay — the dash-offset trick alone can
          // leave a residue of the path's tail on screen.
      ...
      drawTl.to(
        transitionScribblePath,
        { strokeDashoffset: 0, duration: durIn, ease: "power1.inOut" },
        0,
      );
      drawTl.to(
        transitionScribblePath,
    ...
    // shouldn't get it at all. (Clicking the logo still triggers it.)
    const autoRun =
